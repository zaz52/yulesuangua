...
### 2026-07-07 表单体验与结果可读性细节

- 当前任务：跳过微信专项，优先打磨表单体验和结果页可读性。
- 验收标准：
  - 表单未满足提交条件时，页面明确告诉用户缺少什么。
  - 点击排盘后，盘面先出、AI 后出的过程要清楚。
  - AI 解读不再只是相同样式段落堆叠，小标题和正文有明显层级。
  - 移动端无横向溢出，不写本地隐私记录。

### 2026-07-07 表单体验与结果可读性收尾

- 已在 `/divine/:skill` 表单底部加入动态提交提示：未满足提交条件时明确列出缺少字段，满足后提示“信息已补全，可以开始排盘”。
- 已将“正在推演”状态文案改为“盘面已经先行生成，AI 正在结合盘面要点补充文字解读”，让用户理解盘面先出、文字后补的过程。
- 已将 AI 解读从普通段落列表升级为 `answer-section` 结构，支持 `【标题】正文`、`标题：正文` 和短标题行解析，结果区展示为小标题 + 正文的层级卡片。
- 验证通过：`npm run build`。
...
### 2026-07-07 全功能验收与盘面状态区分

- 当前任务：把站点从“能运行”推进到“用户能看懂当前状态”，补全功能验收清单，并明确区分未提交的示例预览盘与提交后的专属结果盘。
- 成功标准：
  - 每个主要功能有清晰的状态记录：真实算法、规则 MVP、文化工具、是否有盘面、是否有固定栏目、隐私状态。
  - `/divine/:skill` 未提交时显示“示例预览盘/未提交”，提交后显示“已提交结果/真实排盘/规则结果盘/本地兜底盘”。
  - 用户不会再把初始样例盘误认为自己的结果盘。
  - 构建、核心浏览器路径和生产页面验证通过。
- 已完成：
  - 新增 `docs/feature-acceptance-audit.md`，列出核心术法、关系心性、生活工具的当前状态和后续方向。
  - `buildBoard()` 增加 `state`，预览盘标记为 `示例预览`，提交后根据后端 source 显示 `真实排盘`、`规则结果盘` 或 `本地兜底盘`。
  - 预览区文案改为“示例预览盘/未提交”，结果卡文案改为“已提交结果”。
  - 结果盘增加视觉强调，示例盘降低饱和度，便于区分。
### 2026-07-07 周易起卦卦辞库接入

- 当前任务：把 `/zhouyi` 结果页从写死“乾为天/天风姤”升级为根据六爻自动计算本卦、变卦、动爻和结构化卦辞结果。
- 成功标准：
  - 六爻生成后，结果页显示真实计算出的本卦和变卦，不再固定为同一个卦。
  - 结果页包含卦辞、象辞、动爻爻辞、变卦、白话解析、行动建议和六爻展开。
  - 64 卦名称矩阵完整可运行；经典文本先内置重点卦，其它卦有结构化兜底，后续可逐卦补全。
  - 移动端无横向溢出，不保存问题和结果。
- 已完成：
  - 新增 `frontend/src/domain/zhouyi.js`，封装八卦、64 卦名称矩阵、重点卦经典文本、六爻转本卦/变卦算法。
  - `/zhouyi` 结果页接入 `buildZhouyiReading()`，动态显示本卦、变卦、动爻、卦辞/象辞/爻辞和建议。
  - 结果页新增“查看六爻爻辞”展开区，动爻高亮。
  - 更新 `docs/feature-acceptance-audit.md` 中周易起卦状态。
